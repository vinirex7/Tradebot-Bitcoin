...
def entry_signal(df: pd.DataFrame, regime: RegimeResult, cfg: dict) -> Signal:
    ...
    if regime.score < regime_cfg.get("buy_threshold_reversal", 55):
        return Signal("HOLD", "none", "regime_score_too_low")

    if atr_pct > 0.09:
        return Signal("HOLD", "none", "volatility_too_high")

    if dist_sma200d > regime_cfg.get("no_buy_if_distance_sma200d_above", 0.35):
        return Signal("HOLD", "none", "price_too_far_from_sma200d")

    reversal_breakout = close > _recent_high(
        df,
        entry_cfg.get("reversal_breakout_days", 7),
    )
    ...
    if reversal_ok or (
        ...
        return Signal(
            ...
            min(
                entry_cfg.get("reversal_position_fraction", 0.25),
                risk_cfg.get("max_exposure", 0.60),
            ),
        )

    breakout = close > _recent_high(
        df,
        entry_cfg.get("breakout_lookback_days", 20),
    )

    volume_ok = row.get("volume", 0) > (
        entry_cfg.get("breakout_volume_multiplier", 1.2)
        * row.get("vol_ma20", 10**99)
    )

    trend_ok = (
        regime.score >= regime_cfg.get("buy_threshold_trend", 70)
        and sma200 > 0
        and close > sma200
        and ema50 > sma200
        and m90 > 0
    ...
    pullback_ok = (
        trend_ok
        and close >= ema20
        and prev["close"] < prev.get("ema20d", prev["close"])
    )
    ...
    if pullback_ok:
        return Signal(
            ...
            entry_cfg.get("trend_position_fraction", 0.50),
        )
...
def exit_signal(df: pd.DataFrame, position: dict, cfg: dict) -> Signal:
    ...
    # Distribution risk DESATIVADO na V3.
    # No backtest, essa regra fazia o bot vender cedo demais durante bull markets.
    #
    # Regra antiga:
    #
